    ...
    public void mousePressed(java.awt.event.MouseEvent mouseEvent) {
      ...
      if (OneTileMoveVector.checkValidity(x-oldX,y-oldY)){
          try{
              ...
              if(trackBuilder.buildTrack(new IntPoint(oldX,oldY), v)==true){
                  message="ok";
              }
              else{
                   message="illegal track configuration";
              }
          
               tableModel.fireTableCellUpdated(oldY,oldX);
               tableModel.fireTableCellUpdated(y,x);
               TextMessageHandler.sendMessage("Lay track from "+oldX+", "+oldY+v.getVectorName()+": "+message);  
