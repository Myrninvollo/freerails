...
public class TerrainMapImpl extends java.lang.Object implements TerrainMap {
    ...
    public TerrainMapImpl(URL url, TerrainTileTypesList terrainTileTypesList) throws FreerailsException {
        try {
            System.out.println("\nLoading map " + url);

            java.awt.Image map = (new javax.swing.ImageIcon(url)).getImage();
            this.mapRect = new java.awt.Rectangle(0, 0, map.getWidth(null), map.getHeight(null));
            ...
            this.mapArray = mapBufferedImage.getRGB(0, 0, mapRect.width, mapRect.height, null, 0, mapRect.width);
        } catch (Exception e) {
    ...
    public int getTerrainTileType(int x, int y) {
        if (mapRect.contains(x, y)) {
            ...
            throw new java.lang.IllegalArgumentException("Tried to get tile from a point outside the map");
        }
    }
    ...
    public String getTerrainTypeName(int x, int y) {
        return terrainTileTypesList.getTerrainName(this.mapArray[y * mapRect.width + x]);
    }
    ...
    public TerrainMapImpl(BufferedImage map, TerrainTileTypesList terrainTileTypesList) {
        this.mapRect = new Rectangle(map.getWidth(), map.getHeight());
        this.mapArray = map.getRGB(0, 0, mapRect.width, mapRect.height, null, 0, mapRect.width);
     ...
     */
    public TerrainMapImpl(int w, int h, TerrainTileTypesList terrainTileTypesList) throws FreerailsException {
        try {
            ...
            throw new jfreerails.common.exception.FreerailsException("Error making map");
        }
    }
}
