...
final public class ImageSplitter extends java.lang.Object {
    ...
    private java.awt.GraphicsConfiguration defaultConfiguration = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment()
                                                                                              .getDefaultScreenDevice()
                                                                                              .getDefaultConfiguration();
    private int subGridYoffset = 0;
    private int tileGridWidth;
    private int tileGridHeight;
    private int gridXAxisOrigin;
    private int gridYAxisOrigin;

    public Image getTileFromSubGrid(int gridX, int gridY) {
        Image tile = getTile(((gridXAxisOrigin +
                ((subGridXOffset + gridX) * tileGridWidth))),
                ((gridYAxisOrigin + ((subGridYoffset + gridY) * tileGridWidth))),
                tileGridWidth, tileGridHeight);
    ...
    public Image getTileFromGrid(int gridX, int gridY) {
        Image tile = getTile(((gridXAxisOrigin + (gridX * tileGridWidth))),
                ((gridYAxisOrigin + (gridY * tileGridWidth))), tileGridWidth,
                tileGridHeight);
    ...
    public Image getTile(int x, int y, int tileWidth, int tileHeight) {
        try {
            java.awt.image.BufferedImage tileImage = defaultConfiguration.createCompatibleImage(tileWidth,
